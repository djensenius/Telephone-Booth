...
- `--ssh` — enables Tailscale SSH so you can `ssh telephone-booth` from any
  device on your tailnet without managing SSH keys. **Note:** Tailscale SSH is
  *additive* — it does not disable your existing SSH daemon (sshd) on port 22.
  Local LAN SSH access (e.g., `ssh pi@telephone-booth.local`) continues to
  work normally. Tailscale SSH just provides an additional, more convenient
  way to access the device over your tailnet.
